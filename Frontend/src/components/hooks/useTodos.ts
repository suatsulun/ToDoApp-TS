...
export const useTodos = () => {
    ...
    useEffect(() => {
        const fetchTodos = async () => {
            try {
                const statusQuery = activeFilters.length > 0 ? `&status=${activeFilters.join(',')}` : ''
                const response = await fetch(`${API_BASE_URL}/api/todos?limit=${pageSize}&sort_order=${sortOrder}&page_number=${currentPage}${statusQuery}`)
